...
export async function DELETE(req, { params }) {
    try {
        ...
        if (isNaN(idNum)) return NextResponse.json({ message: "Invalid postId" }, { status: 400 });

        const deletePost = await Posts.findOneAndUpdate({ postId: idNum }, { $set: { isDeleted: true } }, { new: true });

        if (!deletePost) return NextResponse.json({ message: "Post Not Found" }, { status: 400 });

        return NextResponse.json({ message: "Post Deleted Successfully" }, { status: 200 });
    } catch (error) {
        // console.log(error);
        return NextResponse.json({ message: "Error in deleting posting", error: error.message }, { status: 500 });
    }
}